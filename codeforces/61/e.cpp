...
int main()
{
  ...
  nextnu+=10;
  vi mini(n,0);
  FenwickTree simple = FenwickTree(nextnu + 5);
	FenwickTree doble = FenwickTree(nextnu + 5);
  ll res = 0;
  for(int i=0;i<n;i++){
    int numi = nextnu - mapa[nums[i]];
    simple.update(numi,1);
    doble.update(numi, simple.rsq(numi-1));
    res += doble.rsq(numi-1);
  }
  cout << res << endl;
	return 0;
}
